...
- **Priority:** High
- **Implemented:** ✅ Subscription card as labeled columns (Plan / Status / Renews or Trial ends); a **"Usage this month" section with usage-vs-allowance meters** across all nine metered resources, sorted by how close each is to its limit, showing the top three with a "Show more" that expands the card in place (amber past 80%, red past 95%) so credits are bought next to the numbers that justify them, with a plain-language note that overages draw on credits rather than switching the brand off; plans as a compact cell row using the real tiers from the codebase's `pricing.md` (Free ₦0 / Starter ₦2,000 / Growth ₦5,000 / Scale ₦15,000, with monthly credits and app allowances), current plan outlined, Choose/Switch actions, plus a Top up credits action and a Monthly credits column on the subscription (the platform's billing is prepaid credits per `payment-system-plan.md` — no card on file); the usage list uses the rate card's nine resources and its enforcement rule (credits exhausted → usage pauses until top-up or upgrade); payment history list with Paid/Failed chips and a proper empty state. (Checkout intentionally shows a "not wired up in this prototype" toast — see Non-UI-Issues.md.)
...
- **Revised 2026-09-05 (owner decision):** the phone drawer is retired. Inside a brand, phones get an iOS-style bottom tab bar — Overview, Billing, Finances, Team, Settings — and the account avatar sits top-right on every phone screen, opening a sheet with the brand switcher, New brand, Account settings and Sign out (the avatar was briefly the fifth tab; the owner moved it to the top bar so it never changes place between account-level and brand-level screens). Marketplace is reached from the Overview hub cards. The top bar becomes back chevron + brand name + avatar on brand pages, and wordmark + avatar elsewhere. Rationale and real-product references are in Dashboard-Design-Rules.md §3.1.
- **Priority:** Medium
...
### 15a. Account menu, role management and brand roles (parity pass, 2026-09-05)
- **Found in production:** the sidebar avatar opens a menu (brand switcher, New brand, Account settings at `/settings`, Sign out with a "Sign out of BrandsApp?" confirmation). The Team page offers roles Admin / Owner / Member (view only), a per-row role change, remove, and cancel-invitation. None of this existed in the prototype.

...
- **Account menu:** the account row in the foot (avatar, name, email, up/down caret) is a menu trigger. It opens upward with "New brand", a divider, "Account settings" (`/settings`) and "Sign out". It does not list brands — the sidebar already does, and the same thing is never shown twice on one screen (owner decision, 5 Sep 2026). Sign out confirms in a modal ("Sign out of BrandsApp?" · Stay signed in / Sign out). The same menu lives in the mobile drawer. In the collapsed rail the menu opens beside the avatar.
